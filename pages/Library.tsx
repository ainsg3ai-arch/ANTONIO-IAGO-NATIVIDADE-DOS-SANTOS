...
import { Search, Play, X, Flame, Plus } from 'lucide-react';
...
export const Library: React.FC = () => {
  ...
  const filtered = EXERCISE_DATABASE.filter(ex => 
    (filter === 'Todos' || ex.muscleGroup === filter) && 
    ...
    <div className="p-6 min-h-screen bg-ains-black pb-24">
              ...
              <div key={ex.id} onClick={() => setSelected(ex)} className="bg-zinc-900 border border-zinc-800 hover:border-ains-primary/50 transition-colors cursor-pointer group flex rounded-sm overflow-hidden">
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-center">
                      ...
                      <div className="flex gap-2">
                          ...
                          <span className="text-[10px] font-bold bg-zinc-800 text-zinc-400 px-2 py-1 uppercase">{ex.difficulty}</span>
                      </div>
                  </div>
              </div>
          ))}
      ...
      {selected && (
          <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4">
              <div className="bg-zinc-900 border border-zinc-700 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-sm">
                  <div className="relative aspect-video">
                      <YouTubeEmbed url={selected.videoUrl} title={selected.name} placeholder={selected.videoPlaceholder} autoPlayOnLoad />
                      <button onClick={() => setSelected(null)} className="absolute top-2 right-2 bg-black/50 p-2 text-white hover:text-red-500"><X /></button>
                  </div>
                  <div className="p-6">
                      <h2 className="text-2xl font-display font-bold uppercase text-white mb-4">{selected.name}</h2>
                      <div className="flex gap-4 mb-6 text-sm font-mono text-zinc-400">
                          <div className="flex items-center"><Flame size={16} className="text-orange-500 mr-2"/> {selected.caloriesPerMinute} KCAL/MIN</div>
                          <div className="uppercase">{selected.muscleGroup}</div>
                      </div>
                      <p className="text-zinc-300 leading-relaxed mb-6 text-sm">{selected.description}</p>
                      <button onClick={() => setSelected(null)} className="w-full bg-zinc-800 text-white font-bold py-4 uppercase hover:bg-zinc-700 rounded-sm">Fechar</button>
                  </div>
              </div>
          </div>
      )}
